...
def save_phones():
	...
	while True:
		...
		for i in range(int(len(fav_phones)/2)):
			print(str(i) +" - "+fav_phones[i*2] + " " + fav_phones[i*2+1])
		print("\n1 - Добавить номер\n2 - Удалить номер\n\n\x1b[1;31m0 - Назад\x1b[0m")
		_menu = input()
		if _menu == "0": break
		if _menu == "1":
			fav_phones.append(input("Введите номер: "))
			fav_phones.append(input("Введите название контакта: "))
		if _menu == "2":
	 		delete_phones = int(input("Введите номер телефона, который вы хотите удалить (например 1): "))
...
def bomb():
	...
	try:
		_count = int(input('Количество сообщений: (1000 по умолчанию)'))
	except:
		_count = 1000
	...
	def screen():
		print(banner)
		print("\n\x1b[31mДля остановки нажмите Ctrl+Z\x1b[0m\n")
	...
	print(banner+'\nРезультат:\n\nУдачно ' + str(_count_finish) + ' из ' + str(_count))
	if set[0] == 1.0: exit()
	else: input("Бомбинг завершён, нажмите ENTER для выхода в\nглавное меню")
...
def info():
	...
	print(banner+"\nВерсия "+str(version)+"\n\n\x1b[3;31mБомбер создан только для развлекательных целей. За все действия, что вы с ним проводите, отвечаете только вы!\x1b[0m\n\n\nНажмите ENTER вернуться назад")
	input()
...
\x1b[0m                   
	...
	"""
	print(banner)
	menu = input("1 - \x1b[4mНачать бомбинг\x1b[0m\n2 - Номера в избранном\n3 - Информация о бомбере\n\n\x1b[1;31m0 - Выход\x1b[0m\n")
	if menu == "0": exit()
	if menu == "1": bomb()
	if menu == "68": settings()
	if menu == "2": save_phones()
	if menu == "3": info()
